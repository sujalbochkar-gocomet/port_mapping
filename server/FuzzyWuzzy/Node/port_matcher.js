...
class PortMatcher {
  constructor(portsData) {
    ...
    this.ignoredKeywords = new Set([
      "Harbor", "Seaport", "Wharf", "Dock", "Jetty", "Anchorage", "Haven", "Marina", "Harborage", "Mooring", "Landing", "Boatyard", "Roadstead", "Dockyard", "Quay", "Pier", "Basin", "Inlet", "Cove", "Bay", 
      "Terminus", "Depot", "Station", "Concluding", "Final", "Last", "Closing", "End", "Terminating", "Ultimate", "Extreme", "Endmost", "Nethermost", "Outmost", "Utmost","Terminal","Port"
    ]
    );
  }
  ...
  calculateAccuracy(input, target) {
    ...
    if (matchingWords.length > 0) {
      const inputWordPositions = matchingWords.map(word => inputWords.indexOf(word));
      const targetWordPositions = matchingWords.map(word => targetWords.indexOf(word));
      
      // Calculate how well word positions match
      const positionMatches = inputWordPositions.filter((pos, idx) => 
        Math.abs(pos - targetWordPositions[idx]) <= 1
      );
      
      positionConfidence = (positionMatches.length / matchingWords.length) * 100;
    }
  ...
  exactFullNameSearch(inputString) {
    ...
    const results = this.portsData.filter(port => {
      const mainPortName = this.normalizeString(port['Main Port Name']);
      const alternatePortName = this.normalizeString(port['Alternate Port Name']);
      ...
      else if (alternatePortName === normalizedInput) matchType += '_alternate_port';
  ...
  fullNameSearch(inputString) {
    ...
    const results = this.portsData.filter(port => {
      ...
      const mainMatch = inputWords.every(word => mainPortName.includes(word));
      const alternateMatch = inputWords.every(word => alternatePortName.includes(word));
      const unLocodeMatch = inputWords.every(word => unLocode.includes(word));
      ...
      let matchType = "";
      if (inputWords.every(word => mainPortName.includes(word))) matchType += '_main_port';
      else if (inputWords.every(word => alternatePortName.includes(word))) matchType += '_alternate_port';
      else if (inputWords.every(word => unLocode.includes(word))) matchType += '_unlocode';
  ...
  findByWordSearch(inputString) {
    ...
    for (const word of inputWords) {
      const matchingPorts = this.portsData.filter(port => {
        ...
        const mainPortName = this.removeSpecialCharacters(port['Main Port Name']);
        const alternatePortName = this.removeSpecialCharacters(port['Alternate Port Name']);
        const unLocode = this.removeSpecialCharacters(port['UN/LOCODE']);

        // Split into words and filter out ignored keywords
        const mainWords = mainPortName.split(/\s+/).filter(w => !this.ignoredKeywords.has(w));
        const alternateWords = alternatePortName.split(/\s+/).filter(w => !this.ignoredKeywords.has(w));
        const unLocodeWords = unLocode.split(/\s+/).filter(w => !this.ignoredKeywords.has(w));

        // Check if the current word matches any of the searchable words
        return mainWords.includes(word) || alternateWords.includes(word) || unLocodeWords.includes(word);
      });
      ...
      if (matchingPorts.length > 0) {
        const results = matchingPorts.map(port => {
          ...
          let matchType = "";
          const mainWords = this.removeSpecialCharacters(port['Main Port Name']).split(/\s+/).filter(w => !this.ignoredKeywords.has(w));
          const alternateWords = this.removeSpecialCharacters(port['Alternate Port Name']).split(/\s+/).filter(w => !this.ignoredKeywords.has(w));
          const unLocodeWords = this.removeSpecialCharacters(port['UN/LOCODE']).split(/\s+/).filter(w => !this.ignoredKeywords.has(w));

          if (mainWords.includes(word)) matchType += '_main_port';
          else if (alternateWords.includes(word)) matchType += '_alternate_port';
    ...
    const results = fuseResults
      .map(result => {
        ...
        if (matches.length > 0) {
          ...
          else if (matchedKey === 'Alternate Port Name') matchType += '_alternate_port';
  ...
  cascadingSearch(inputString, fuzzyThreshold = 60, maxResults = 3) {
    ...
    const fuzzyMatches = this.fuzzySearch(inputString, fuzzyThreshold, maxResults);
