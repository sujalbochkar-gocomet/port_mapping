...
import { Port, Shipment, statusPort } from "../types/types";
...
const ShipmentForm = () => {
    ...
    (term: string, isPol: boolean) => {
      ...
      timerRef.current = setTimeout(async () => {
        try {
          ...
          if (results.length === 0) {
            // Create a temporary port if no results are found
            const tempPort = getTempPort(term, portType);
            results = [
              {
  ...
  const addShipment = async () => {
    ...
    if (selectedPol.port.id === selectedPod.port.id) {
      toast.error(
        "POL and POD cannot be the same port. Please select different ports."
      );
      return;
    }
...
const getTempPort = (term: string, portType: string): Partial<Port> => {
  return {
    _id: `temp-${Date.now()}`,
    id: `temp-${Date.now()}`,
    name: term,
